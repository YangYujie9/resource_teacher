            ...
            <el-tabs stretch v-model="activeType" @tab-click="handleClick">
              ...
              <el-tab-pane label="按考点" name="knowledge">
                <div class="tree-class">
                  <pointTree chooseType="knowledge" :grade="form.grade.key" :subjectCode="subjectCode"  @getCheckedNodes="getCheckedKnows" ref="knowledgeTree"></pointTree>
                </div>
              </el-tab-pane>
            </el-tabs>
          </div>
      ...
      <div slot="right">
        <div class="content-class">
          ...
          <div class="form-class" v-show="questionType=='SingleChoose' ||questionType=='MultipleChoose'||questionType=='BoolenQuestion'">
            ...
            <div v-if="questionType=='SingleChoose' ||questionType=='MultipleChoose'">
            ...
            <div v-if="questionType=='BoolenQuestion'">
              <Ueditor :writeMsg="defaultMsgA" :id="ueditorY" :config="configA" ref="ueY" v-show="currentOption.label == 'Y'"></Ueditor>
              <Ueditor :writeMsg="defaultMsgA" :id="ueditorN" :config="configA" ref="ueN" v-show="currentOption.label == 'N'"></Ueditor>
...
export default {
  ...
  computed: {
    ...
    //     id = this.$refs.filter.filter.volume.oeseId
    //   }else {
    //     id = ''
    //   }
    //   console.log()
  ...
  mounted() {
    ...
    // console.log(this.form.subjectId,this.subjectCode)
    this.init()
  ...
  methods: {
      ...
      if(this.query && this.query.paperId) {
        ...
        if(this.gradeList.length) {
          ...
          let grade = this.gradeList.filter(item=>{
            return item.key.indexOf(this.actualPaper.grade)>-1
          })
    ...
    changeOption() {
      console.log(this.questionType)
        if (this.questionType == "SingleChoose" || this.questionType == "MultipleChoose") {
          ...
          this.optionList = [
            { label: "Y", check: true, content: "" },
            { label: "N", check: false, content: "" }
          ];
          this.currentOption = this.optionList[0]
          this.form.optionNum = 2;
          this.form.relOptionNum = 1;
    ...
    save_prev(flag) {
      ...
      if(this.questionType == "SingleChoose" ||this.questionType == "MultipleChoose" ||this.questionType == "BoolenQuestion") {
        this.optionList.forEach(item=>{
        ...
        if (data.status == "200") {
          ...
          if(this.isActual) {
            ...
            if(this.actualPaper.questionType == '综合题') {
              ...
              this.questionType = this.typeList[0].id;
